...
**If your router is a TP-Link Omada Gateway** (not just an Omada-managed switch or access point), current firmware has a feature built for exactly this: **DNS Override**, under **Settings** > **Services** > **DNS Proxy**.
Enabling it and pointing it at your Pi-hole's IP transparently redirects DNS requests bound for any other server to Pi-hole instead, for the networks you select, with no per-device configuration.
It requires an actual Omada Gateway unit (not all Omada hardware qualifies) running controller version 5.15.20 or later.
Check **Settings** > **Services** > **DNS Proxy** in your own controller to see if it's available before assuming it isn't.
