...
description: "How to set Pi-hole as your network's DNS server via router DHCP, and force devices with hardcoded DNS (Chromecast, smart TVs) to use it with iptables NAT rules."
...
1. Verify the rules were added:
   ...
   ```shellsession "192.168.1.25" title="Example output, with YOUR-PI-IP replaced"
   Chain PREROUTING (policy ACCEPT)
   num  target     prot opt source               destination
   1    DNAT       udp  --  anywhere             anywhere             udp dpt:domain to:192.168.1.25
   2    DNAT       tcp  --  anywhere             anywhere             tcp dpt:domain to:192.168.1.25
   ```
...
- If you added the iptables rules, `sudo iptables -t nat -L PREROUTING --line-numbers` still lists both DNAT rules.
