...
1. Edit the file to include:
   ...
   ```shell title="~/.pi-hole-update.sh" showLineNumbers
   #!/bin/zsh
...
1. Log in to the controller and select the site's name.
1. Select the **Settings** cog then **Wired Networks** and select the edit icon for the network name that should use the Pi-hole.
