...
To do this, you configure Access Control List (ACL) grants based on how you access your Pi and Claude Code (outlined in [Choose Your Setup](/docs/pi-hole/claude-code-access/#choose-your-setup)).
In this case, ACL is basically a finer-grained way to grant or restrict Linux user, group, and file access.
...
```shellsession title="Output in Claude Code"
...
That means a `cli_pw` session could import a Teleporter archive and overwrite `pihole.toml` outright, including DNS settings, blocking status, DHCP config, and upstream servers.
...
When Claude Code runs as your login user, its blast radius is everything you can read: your SSH keys, your dotfiles, your shell history.
