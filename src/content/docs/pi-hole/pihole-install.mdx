...
- A static IP. The installer prompts "Static IP Needed" and expects one already set. If you haven't set one yet, see [Set a Static IP Address](/docs/pi-hole/install-configure/#set-a-static-ip-address) first.
...
1. Update the system and install dependencies:
   ...
   This can take 10 to 20 minutes, even on a freshly installed OS.
...
1. The installer prompts you with warnings and options. Select:
   ...
   - **Blocklists**: **Yes** to include the default list.
     You'll change it to a more curated set of lists later.
...
1. Create the Pi-hole-specific unbound configuration:
   ...
   server:
       ...
       # Disable capitalization randomization
       use-caps-for-id: no
...
1. Verify that it's running on port 5335:
...
<details>
<summary>Add Netdata if you want system-level metrics alongside your Pi-hole data.</summary>
...
1. Access the dashboard over an SSH tunnel:

   ```shell title="From your device"
   ssh -L 19999:localhost:19999 pi-admin@pi-hole.local
   ```

   Netdata's local dashboard has no login of its own.
   Anyone who can reach the port can see CPU and memory usage right away, and the Fail2Ban jail status too once you enable the collector below.
   A tunnel keeps the dashboard reachable only from the device you connect from, with no extra port open on the Pi.

   <details>
   <summary>If you want it reachable from other devices on your network instead</summary>

   ```shell title="From the Pi"
   sudo ufw allow 19999/tcp comment 'Netdata dashboard'
   ```

   This opens the dashboard, unauthenticated, to every device that can reach the Pi.
   Only open it if you accept that tradeoff.

   </details>

1. Open the dashboard at `http://localhost:19999` (over the tunnel) or `http://pi-hole.local:19999` (if you opened the port instead).
