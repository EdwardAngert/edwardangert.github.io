...
macOS users can install both tools using [Homebrew](https://brew.sh/).
...
1. **Customisation: SSH authentication**: Enable SSH and select **Use password authentication**.
   - Optionally, [use public key authentication](https://www.raspberrypi.com/documentation/computers/remote-access.html#configure-ssh-without-a-password) for more secure access.
     ...
     1. Back in Imager, select **Allow public-key authentication only**, then **Add SSH key**, and paste the contents of the `.pub` file (find it with `cat ~/.ssh/id_ed25519.pub`).
...
1. Create `/etc/fail2ban/jail.local`:
   ...
   `banaction = ufw` tells Fail2Ban to use UFW to block IPs instead of managing iptables rules directly (iptables is an alternative way to manage network access to the Pi), keeping all firewall rules in one place.
...
1. Set the static IP, gateway, and DNS.
   ...
   - If you do get locked out, you'll need to access the Pi directly, with a monitor and keyboard.
