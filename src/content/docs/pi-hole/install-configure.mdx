...
1. **Customisation: Choose Wi-Fi** to configure the wireless network connection.
   - Select **Next** if you plan to use the Ethernet connection.
1. **Customisation: SSH authentication**: Enable SSH and select **Use password authentication**.
   - Optionally, [use public key authentication](https://www.raspberrypi.com/documentation/computers/remote-access.html#configure-ssh-without-a-password) for more secure access.
     ...
     1. Copy the contents of the `.pub` file to the clipboard:
     ...
     1. Back in Imager, select **Allow public-key authentication only**, then **Add SSH key**, and paste the contents of the `.pub` file.
...
1. If you configured Wi-Fi and also plugged the Pi in via Ethernet, your Pi will have two IP addresses.
   ...
   To make it easier to [enable network-wide blocking](/docs/pi-hole/network-level-blocking/#set-up-network-wide-pi-hole-on-a-tp-link-omada-controller) later, identify the Pi's IP address for the connection it will use.

   Confirm the IP address from inside the SSH session on the Pi:

   <Tabs syncKey="net-connection">
...
1. Add the following to `/etc/ufw/applications.d/pihole` to group the required ports into a single app profile rule.
   ...
   That creates the file and opens the nano editor so you can add the rule.
...
1. Create a new configuration file, `/etc/ssh/sshd_config.d/60-disable-x11-forwarding.conf`, to disable it:

   ```java title="/etc/ssh/sshd_config.d/60-disable-x11-forwarding.conf"
   X11Forwarding no
   ```
...
1. Debian's package defaults already enable security-origin updates and leave automatic reboots off.
   ...
   For a DNS server, it's usually safer to reboot manually for minimum interruption.
...
It's not what Pi-hole serves to the rest of your network.
That's a separate setting you choose on the [next page](/docs/pi-hole/pihole-install/) when you pick Pi-hole's upstream DNS provider.
...
1. Set the static IP, gateway, and DNS.
   ...
   - This setting uses the shell variable `CON` to make it easier to copy and paste the command consistently.
     The first line finds the connection's name, and sets it as the variable.
