...
title: "Add Blocklists and Allowlists to Pi-hole"
description: "Choose a HaGeZi blocklist tier for Pi-hole v6, fix Disney+, Hulu, and other streaming services it breaks, and control blocking per device with groups."
...
- [Fix Streaming Services and Other Broken Sites](#fix-streaming-services-and-other-broken-sites)
...
**HaGeZi** aggregates over 200 upstream sources including OISD, EasyList, EasyPrivacy, URLhaus, and Phishing Army into tiered lists you can dial up or down.
...
Some list maintainers still use some of the outdated language in their descriptions and in the list URLs.
:::
...
1. Optional: Choose any supplementary lists (use with either tier):
   ...
   - HaGeZi Bad Hoster
     ...
     Blocks entire hosting providers that repeatedly host malware in user-uploaded content, not individual bad domains.
     Legitimate sites sharing that hosting get blocked too, with no built-in path to unblock just the one you need.
     HaGeZi scopes this list for high-security environments, not casual use.
     Skip it unless you're prepared to hand-unblock the occasional legitimate site yourself.
...
- HaGeZi [NRD/DGA list](https://github.com/hagezi/nrd)
  - Blocks newly registered and algorithmically generated domains, a category that malware command-and-control infrastructure relies on.
    This can improve your security posture, but can lead to more false positives.
...
Pi-hole v6 offers two ways to allowlist:
...
Instruct Pi-hole to allow individual domains.
...
See [Troubleshoot a Broken Service](/docs/pi-hole/troubleshooting/#troubleshoot-a-broken-service) for tips.
...
If your service isn't listed below, see [Troubleshoot a Broken Service](/docs/pi-hole/troubleshooting/#troubleshoot-a-broken-service) for how to find the blocked domain yourself.
...
If you block the shared video-delivery domain itself, you break playback entirely rather than removing the ads, because the ad and the show are the same stream.
There's no ad-only piece of that stream to leave blocked while allowing the rest.
...
The likely mechanism: both apps check in with an analytics/QoS SDK (Conviva, Braze) before deciding whether to insert a targeted ad, and appear to skip the ad rather than fall back to blocking playback if that check fails.
...
Paramount+ uses Google's Dynamic Ad Insertion, documented by Google as stitching ad pods directly into the content stream server-side.
Fixing playback requires allowlisting Google ad infrastructure domains, which defeats the purpose of Pi-hole.
...
Compiled from the [ozankiratli streaming whitelist gist](https://gist.github.com/ozankiratli/801ba17705e7f2a904d2e443af5a64f8) and [Google's Dynamic Ad Insertion documentation](https://developers.google.com/ad-manager/dynamic-ad-insertion/pod-serving). *(Accessed July 28, 2026.)*
...
- **In a browser:** an extension like [uBlock Origin](https://github.com/gorhill/uBlock?tab=readme-ov-file#ublock-origin-ubo).
- **On a TV:** [SmartTube](https://smarttube.app/), an open-source replacement YouTube client for Android TV, Google TV, Fire TV, and Nvidia Shield. It removes ads and integrates SponsorBlock to skip in-video sponsor segments. It has to be sideloaded, because it is not in the app stores. *(Accessed July 28, 2026.)*
...
None of these are DNS-level fixes.
They replace or modify the client instead, which is the only layer where YouTube's ads and content can still be told apart.
...
<Verified date="2026-04" method="community" />
...
Compiled from [techjunctions' Samsung TV ad blocker guide](https://techjunctions.com/samsung-tv-ad-blocker/) (updated April 2026) and [Perflyst/PiHoleBlocklist issue #10](https://github.com/Perflyst/PiHoleBlocklist/issues/10). *(Accessed July 28, 2026.)*
...
Fire TV uses a captive-portal check to decide whether it has internet.
...
`fireoscaptiveportal.com` comes from [hkamran80's smart TV blocklist](https://github.com/hkamran80/blocklists), which removed it after a user reported it breaking their Fire Stick with a network error.
Neither captive-portal domain appears in [the current `smart-tv.txt`](https://raw.githubusercontent.com/hkamran80/blocklists/main/smart-tv.txt), which is the evidence for allowing them.
...
*(Sources and domain registrations re-checked August 23, 2026.)*
Not tested on my own hardware.

#### Roku

<Verified date="2026-07" method="community" />

Roku is the most awkward device on this list, because it fights back in two ways.
...
See [Force Devices That Ignore Router DNS to Use the Pi-hole](/docs/pi-hole/network-level-blocking/#optional-force-devices-that-ignore-router-dns-to-use-the-pi-hole).

**Some ads aren't DNS-delivered at all.** The home screen banner and screensaver ads can be reduced but not removed, because parts of them ship with the platform rather than being fetched per-view.
Roku's own **Settings** > **Privacy** options cut more of them than any blocklist does.
...
Blocked telemetry domains are also what cause apps to load with no thumbnails, or video that won't start.
If that happens, use the query log method above rather than guessing, because the exact domains vary by region and Roku model.

Compiled from the [ozankiratli streaming whitelist gist](https://gist.github.com/ozankiratli/801ba17705e7f2a904d2e443af5a64f8) and [cybernews' Roku ad blocking guide](https://cybernews.com/best-ad-blockers/how-to-block-ads-on-roku-tv/). *(Accessed July 28, 2026.)*
Not tested on my own hardware.
...
1. Go to **Clients** and search for or select the device from the **Known clients** dropdown.
   Add a comment in the **Client description** to help identify the device later.

1. Under **Group assignment**, uncheck `Default` to remove it from the group.
...
1. Under **Group assignment**, unselect `Default` and select the new group.
   ...
   | Domains registered in the last week and domains that look randomly generated. More info at [hagezi/nrd](https://github.com/hagezi/nrd#readme). | `https://cdn.jsdelivr.net/gh/hagezi/nrd@latest/adblock/dga7.txt` |

   <Aside type="note" title="Blocking YouTube via DNS is all-or-nothing" icon="youtube">
    There's no way to allow educational content while blocking everything else.

    If your goal is "no YouTube at all," add `youtube.com` as a domain to the group's blocklist.
    If your goal is "safe YouTube," DNS isn't the right tool.
    Use YouTube's built-in parental controls or a browser extension instead.
   </Aside>
...
- Gravity shows a domain count in the dashboard (run `sudo pihole -g` if it's empty)
...
- If you run into issues, see [Common Pi-hole Issues](/docs/pi-hole/troubleshooting/#blocklists-and-allowlists).
