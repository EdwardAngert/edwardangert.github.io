---
slug: docs/pi-hole
title: "Pi-hole v6 Setup Guide for Raspberry Pi"
description: "Set up Pi-hole v6 on Raspberry Pi for network-wide ad blocking, with HaGeZi blocklists, streaming allowlists, and optional Tailscale, unbound, and Netdata."
icon: "laptop"
...
When a device on your network tries to access something from the internet, it checks in with Pi-hole first.
Pi-hole checks the address against a list you've configured, and if that resource isn't on the list, it allows the device to connect.
...
- Optional:
  ...
  - More restrictive [groups for specific devices](/docs/pi-hole/block-allow-lists/#block-youtube-and-social-media-for-a-specific-device)
...
If you identify any errors or areas for improvement, [submit a GitHub issue](https://github.com/EdwardAngert/edwardangert.github.io/issues/new) or select **Edit page** at the bottom of any page to suggest a change.
...
- [Raspberry Pi](https://www.raspberrypi.com)
  - You can use another always-on device on your local network, or even a VPS.
    Most of the steps in this guide will still work, but that's outside this guide's scope and some steps might differ.
...
- [Set Up Raspberry Pi OS](/docs/pi-hole/install-configure/)
  - Flash the SD card, harden the OS with UFW and Fail2Ban, set a static IP.
- [Install Pi-hole v6](/docs/pi-hole/pihole-install/)
  - Run the installer, set a password, and optionally add unbound and Netdata.
- [Add Blocklists and Allowlists](/docs/pi-hole/block-allow-lists/)
...
- [Enable Network-Wide DNS Blocking with Pi-hole](/docs/pi-hole/network-level-blocking/)
  - Configure router DHCP so every device uses Pi-hole for DNS, including devices with hardcoded DNS like Chromecast or some smart TVs.
- [Use Pi-hole as a VPN to Block Ads on the Go with Tailscale](/docs/pi-hole/tailscale/)
  - Block ads on your phone and laptop anywhere, remote admin access, no port forwarding required.
- [Pi-hole Maintenance and Advanced Configuration](/docs/pi-hole/maintenance/)
  - Update Pi-hole, export/import backups with Teleporter, tune `pihole.toml`, offload logs to a USB drive.
- [Troubleshooting](/docs/pi-hole/troubleshooting/)
...
- [Give Claude Code Access to Pi-hole Logs and Diagnostics](/docs/pi-hole/claude-code-access/)
  - Grant read-only access to the query database and logs with POSIX ACLs, optionally under a dedicated unprivileged user.
