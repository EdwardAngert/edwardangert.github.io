---
slug: docs/site-mz-failover-diagram
title: "Site Multizone Failover Diagram"
description: "Technical diagram created for Pantheon's multizone failover documentation"
---

Based on feedback from Support and engineers, I created a draft of this diagram and then worked with Pantheon’s design team to \"commission\" the final product.

Originally available at [Pantheon's Site Multizone Failover](https://docs.pantheon.io/multizone-failover) doc.

![Diagram outlines Pantheon's zone-based Multizone Failover architecture](~/assets/images/site-mz-diagram.png)

Link to PNG: [site-mz-diagram.png](~/assets/images/site-mz-diagram.png)
