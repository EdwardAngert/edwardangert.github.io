...
- SD Card Formatter
  - Mac/Windows: https://www.sdcard.org/downloads/formatter/
...
If you configured Wi-Fi and plugged the Pi in via Ethernet, your Pi will have two IP addresses. To make it easier to [enable network-wide blocking](#configure-the-omada-controller-to-enable-network-wide-blocking-with-pi-hole) later, identify the Pi's Ethernet IP address.
...
1. The script will prompt you with a number of warnings and options. When prompted select:
   ...
   - **Static IP Address**: `Yes` and enter the IP address you found in [Find the Pi's IP Address](#find-the-pis-ip-address).
     - Unless you've made specific changes to the network, the gateway address should be the IP of the router or controller. In our case, that's `192.0.2.2.1`.
...
1. Write down the temporary password!
   ...
   The last screen lists the IP and a temporary password for the Pi-hole web interface. Write it down and change it in the next step.
