...
The steps here use the pihole-updatelists script. The official [pihole-updatelists documentation](https://github.com/jacklul/pihole-updatelists#update-pi-holes-lists-from-remote-sources) includes information about other set up and configuration options if you prefer to use Docker or would rather avoid piping to bash.
...
1. Edit the configuration file to add lists:

   ```zsh
   sudo nvim /etc/pihole-updatelists.conf
   ```
